...
if __name__ == "__main__":
    ...
    mat_files = ['./data/scalefree64_sourcedata_ddos1_newV1.mat']
    #mat_files = ['./data/scalefree64_sourcedata_ddos1_R1.mat']
    sourcedata_names1 = ['scalefree64_dst_entropy_ddos1', 'colt_dst_entropy_ddos2', 'smallworld128_dst_entropy_ddos1', 'smallworld128_dst_entropy_ddos2']
    sourcedata_names2 = ['scalefree64_arrival_rate_ddos1', 'colt_arrival_rate_ddos2', 'smallworld128_arrival_rate_ddos1', 'smallworld128_arrival_rate_ddos2']
    label_names = ['scalefree64_data_label_ddos1_block', 'colt_data_label_ddos2_block', 'smallworld128_data_label_ddos1_block', 'smallworld128_data_label_ddos2_block']
    parameters = {'n_clusters': 3}

    for file_list_id in range(len(mat_files)):
        current_mat_file = mode1o.loadmat(mat_files[file_list_id])  # <class 'dict'>
        sourcedata1_block = current_mat_file[sourcedata_names1[file_list_id]]
        sourcedata2_block = current_mat_file[sourcedata_names2[file_list_id]]
        label_block = current_mat_file[label_names[file_list_id]]  # <class 'numpy.ndarray'>
        (node_num, timeslot) = sourcedata1_block.shape
        ...
        for repeat_id in range(repeat_times):
            ...
            sourcedata1_block = sourcedata1_block[:, shuffle_index]
            sourcedata2_block = sourcedata2_block[:, shuffle_index]
            label_block = label_block[:, shuffle_index]

            # 为了方便索引，这里需要对数据复制
            sourcedata1_block = np.tile(sourcedata1_block, (1, 2))
            sourcedata2_block = np.tile(sourcedata2_block, (1, 2))
            ...
            for fold_id in range(folds):
                ...
                train_data1 = sourcedata1_block[:, train_index]
                train_data2 = sourcedata2_block[:, train_index]
                train_label = label_block[:, train_index]
                test_data1 = sourcedata1_block[:, test_index]
                test_data2 = sourcedata2_block[:, test_index]
                test_label = label_block[:, test_index]

                # 将所有时隙的数据给连接起来
                # 在Python中reshape是按照行来变换的，与MATLAB不同，因此需要先做转置
                train_data1 = train_data1.transpose()
                train_data1 = train_data1.reshape(-1, )
                train_data2 = train_data2.transpose()
                train_data2 = train_data2.reshape(-1, )

                train_label = train_label.transpose()
                train_label = train_label.reshape(-1, 1)

                test_data1 = test_data1.transpose()
                test_data1 = test_data1.reshape(-1, )
                test_data2 = test_data2.transpose()
                test_data2 = test_data2.reshape(-1, )

                test_label = test_label.transpose()
                test_label = test_label.reshape(-1, 1)

                (n_samples,) = train_data1.shape
                train_data = np.zeros((n_samples, 2), dtype=float)
                train_data[:, 0] = train_data1
                train_data[:, 1] = train_data2

                (n_samples,) = test_data1.shape
                test_data = np.zeros((n_samples, 2), dtype=float)
                test_data[:, 0] = test_data1
                ...
                for name, algorithm in clustering_algorithm:
                    ...
                    train_pre = re_label(source_data=train_data2.reshape((-1, 1)), label=train_pre, label_num=parameters['n_clusters'])
                    test_pre = re_label(source_data=test_data2.reshape((-1, 1)), label=test_pre, label_num=parameters['n_clusters'])
        ...
        scalefree64_ddos1_performance = {'performance_average_train_KMeans': performance_average_train_KMeans,
    ...
    # save variables
    with open('./result/performance_scalefree64_ddos1_Rtmp.pickle', 'wb') as f:
        pickle.dump([scalefree64_ddos1_performance], f)

    # with open('./result/performance_scalefree64_ddos1V2.pickle', 'rb') as f:
    #     scalefree64_ddos1_performance = pickle.load(f)
    time2 = time.time()
