...
for i=1:state_num
    real_id=find(real_state==i);
    temp_estimate_result=estimate_state(real_id);
    for j=1:state_num
...
%性能指标，第一行为总的准确率，第二行为总的错误率，第三行为每一类的精确率precision，第四行为每一类的召回率recall，第五行为每一类的F1值
performance_index=zeros(5,state_num);
